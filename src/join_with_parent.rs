    ...
    Updates::Item: IntoIterator<Item = (Key, Option<Value>)>,
{
        ...
        ParentUpdates::Item: IntoIterator<Item = (ParentKey, Option<ParentValue>)>,
    {
        ...
        let initial_state = JoinOneToManyState::new(parent_snapshot, snapshot, f);
...
struct JoinOneToManyState<ParentKey, ParentValue, Key, Value, F> {
    snapshot: im::HashMap<ParentKey, (Option<ParentValue>, im::HashMap<Key, Value>)>,
    f: F,
}

impl<ParentKey, ParentValue, ChildKey, ChildValue, F>
    JoinOneToManyState<ParentKey, ParentValue, ChildKey, ChildValue, F>
where
    ParentKey: Clone + Hash + Eq,
    ParentValue: Clone,
    ChildKey: Clone + Hash + Eq,
    ChildValue: Clone,
    F: FnMut(&ChildKey) -> ParentKey,
{
    fn new(
        parents: impl IntoIterator<Item = (ParentKey, ParentValue)>,
        children: impl IntoIterator<Item = (ChildKey, ChildValue)>,
        mut f: F,
    ) -> Self {
        use im::HashMap;
    ...
    fn ingest_update<'a>(
        ...
        update: Either<
            ...
            impl IntoIterator<Item = (ChildKey, Option<ChildValue>)>,
        >,
    ) -> Vec<((ParentKey, ChildKey), Option<(ParentValue, ChildValue)>)> {
        use im::HashMap;
    ...
    fn ingest_update_optional_parent<'a>(
        ...
        (ParentKey, ChildKey),
        Option<(Option<ParentValue>, ChildValue)>,
    )> {
        use im::HashMap;
